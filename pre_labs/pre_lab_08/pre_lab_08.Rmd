...
It's got 24 rows and 18 columns. 
...
**Answer**
First, you mutate the dataframe to include a column with the increase in unaffiliated voters between 2022 and 2024. Then, we're adding it to a new dataframe that is ordered from greatest to least by the change in unaffiliated voters. Then, we're just selecting 2 columns -- the county and difference in unaffiliated voters -- to include in the dataframe, and only the top 10 counties. Then we're showing the new dataframe as a tibble below. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** I just see a blank square at this point
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** There's a bar chart now! It's comparing the growth in unaffiliated voters across counties. 

```{r}
maryland_top_counties |>
  ggplot() +
  geom_bar(aes(x=County, weight=UNAF_DIFF))
#weight = y axis, more or less. 
```
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** Now the bar plot is ordered from least to greatest. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** Now it's horizontal and the county names read much better. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** It got rid of the gray background and changed some lines. 
...
**Task** Run the following code. **Answer** Code has run, packages loaded.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last version of the chart? **Answer** Now it's blue and gray, and the measuring lines have changed. It looks nice. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** There's a title, and the axis names have changed! The data source is cited in the bottom right corner, too. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** There's a tibble showing the total amount of contributions on each day. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** This is way easier to understand than just the dataframe. Now we see a line chart with date on the x axis and # of contributions on the y axis. You can see how the amount of contributions changed over time, and the spike at the very end around July really stands out. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** The counts are labled in increments of 200 on the y axis, and every week is labeled with a date on the x axis. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** The date labels are angled so that they look nicer and are easier to read. 
