...
**Task** Run the following code. Describe the output in the space below: what kind of information does it contain? **Answer** It's a normal-looking dataset with columns and rows, and includes a bunch of geographic data. It's got precinct and polling IDs, the name of the precinct, the congress district, the legislative district, and the school. Then, it has columns called "shape length," "shape area," and "shape". The length and area columns are full of big numbers and the shape column says either "multipolygon" or "multisurface" and has more big numbers. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** Yeah this is super cool. It gave us a very simple map of PG County with longitude and latitude marked along the x and y axes. The county map is divided up into these precincts. Most of them are gray but some are white. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** I've got tigris now. R is telling me which version of R this was built under and says you can use it to cache data? 
...
**Task** Run the following code. Describe the output in the space below. **Answer** This code pulls up a dataset with what looks like all the counties in the US. It's got the name of the county and a bunch of geographic data, including some of the geometric/lat/long data from the previous dataset. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** Looks like it's got all the same info but now it's just for the 23 MD counties and Baltimore. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** This is a map of Maryland, with counties outlined and longitude/latitude marked on the axes. Apparently the Chesapeake is included in county boundaries. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** This map looks a little cleaner, especially around the edges. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** This lists voter turnout in PG County by precinct and vote type. I bet the precinct ID numbers match the ones in the earlier dataset.  
...
**Task** Run the following code. Describe the output in the space below. **Answer** It's joined by precinct ID and has info from both of the datasets. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** Now all the precincts are different shades of blue and white! they look pretty. The colors are based on the total number of eligible voters. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** This is voter data for each MD county. It lists the amount of voters in each party and the total number of voters for each county. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** This dataset has population totals for each MD county. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** Census data that includes the total population for each MD county, as well as geometry data. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** The names are different, and it also looks like one is a tibble and the other is a different kind of file? They've got different formats. I think the population column is shared between the two, though. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** Cleaned up now! Each county name is just the county name, matching it to the other data set. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** They're joined! It's got the county, population, voter affiliation, and geometry. But there are NA values for several counties. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** Cleaned up! Should be easier to join now. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** The dataset's got another column now, voters_per_10k. The first 10 rows feature counties with 7-8K voters per 10k people. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** THIS IS COOL. It's a map of MD divided into counties, and each county is a different color based on the number of voters per every 10K people. It's got a legend on the right side. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** The colors are easier to tell apart. I can instantly tell which counties have the highest and lowest numbers of voters per 10K. 
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer** PG County and Montgomery County have some of the lowest voter rates, while counties around them have higher ones. Counties with some of the highest voter rates are on the Eastern Shore. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** Now we're calculating the rate of unaffiliated voters per every 10K people. 
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see, especially on the ends of the scale? **Answer** There are high rates of unaffiliated voters in Anne Arundel, Howard and Frederick counties, and low rates of unaffiliated voters in Baltimore City, PG County, Garrett County, and Somerset County. 
