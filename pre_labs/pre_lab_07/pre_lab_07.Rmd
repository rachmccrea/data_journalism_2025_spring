...
#API NOTES 
API = how computers talk to each other on the internet
basically what's under the hood. 
we want data that's for computers to read, so we use tidycensus, our first API. 
tidycensus is a wrapper on top of the API
...
```{r echo=FALSE}
#census_api_key("GITHUB_DOESNT_GET_MY_API_KEY", install=TRUE)
```
...
View(sf1)
#variable is the name for whatever it is. 
#cache = true stores it on your computer. you don't have to run the api again
```
...
**Task** Run the following codeblock to calculate the percentage change and put the state with the largest percentage growth below. **Answer** Nevada was the state with the largest percentage growth (over 35%)
...
**Task** Run the following codeblock to find the Maryland county with the highest median household income and put the answer below. **Answer** Howard County is the richest in Maryland, with a median income of $140,971
...
Is the difference between Calvert and Montgomery significant? **Answer** NO! Their bars overlap. 

Is the difference between Howard and everyone else significant? **Answer**YES! It has no overlap with other counties. 
...
**Answer**	Looks like it's just 21202 with a median household income of 47848. I looked it up, and it's a section of Baltimore bordering the harbor. 
...
md_zips_with_acs <- md_zips_with_acs |> mutate(min=(estimate-moe), max=(estimate+moe)) %>% arrange(desc(total))
```
...
Where are these zip codes? Who lives there? There are a whole bunch of 208's together in the top zip codes, are they the same geographic area? Why did the zip with the highest median household income only donate $1500? 
I looked up these zip codes -- the 208's are all in Montgomery County. 21723, which had the highest estimated median income, is in Cooksville, Howard County. 
```{r}
