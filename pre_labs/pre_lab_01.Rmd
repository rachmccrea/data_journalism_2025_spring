...
**131395 observations and 24 variations **
...
In the space below, write the data type for the "last_name" column and the "amount" columns (it's in light gray, offset by \< \>). Also offer your best guess as to what the "last_name" column and the "amount" column represent. **data type for last_name is text, it lists the last names of the subjects. the amount column uses numbers instead of text. I bet these numbers are amounts of money -- donations? That would mean last_name would be the last names of the donors. Ok after looking through all the columns it looks like this is a list of political donations ("WinRed" makes sense now). "last_name" is last names of donors and "amount" is the amount donated. **
...
**Answer: it counts the amount of donations on each day. The code groups donations by date using group_by(date), then counts the amount of contributions using count_contribs = n(). I don't know what the 'summarize' command means, but looks like it's connected to the contribution count.**
...
**Answer: this command organizes the days by most contributions. The most contributions came on May 31, with over 8000 contributions.**
...
**Now the contributions are sorted with the memo_text describing where they donated first, then the amount, then the number of contributions to that group or fund.**
...
**Now it's the memo_text, then the number of contributions, then the total amount donated to each fund/group/etc**
...
**Now we have all these summary statistics for this data! We can see who got the most contributions and who got the most money from contributions, but we can also see how much people donated to each campaign on average and the median of that data. That's super cool. The Trump National Committee got the most individual contributions (almost 42K) and the most total money from contributions (1,735,740). **
...
Compared to the previous codeblock, what has changed and how did that affect the result? How is the data sorted? **The data is sorted with highest total campaign contribution per city first. Annapolis donated the most money, followed by Bethesda. It's interesting to note that Hagerstown donated the least amount of money in total and the least amount per person, but had the highest number of contributions. Chevy Chase contributed the most money per person on average.**
