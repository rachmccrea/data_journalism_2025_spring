...
https://datawrapper.dwcdn.net/EnQ2f/1/
#wasn't sure if we were supposed to copy the title/text from the pictures or do our own, I did my own. 
...
md_gov_county <- md_gov_county %>% 
  group_by(County) %>% 
  mutate(total = sum(Cox, Moore, Lashar, Wallace, Harding))
...
Now there are no counties that aren't in Maryland, it's just Maryland on a white background. You can see the different colors in different counties now -- you really couldn't when it was a map of the whole US. PG County is the deepest blue, and it looks like Garrett is the lightest blue. 
...
Now it's got the county name instead of the FIPS code which is great, and the number has gone from having a ton of decimal places to just one. It looks way cleaner and is easier to read. 
...
**Task** Click the "Proceed" button until you get to the "Publish & Embed" tab, then click "Publish Now". Copy the published URL (like you did for the chart above) and paste it below. **Answer** Put the URL of your map here. https://datawrapper.dwcdn.net/WprPR/1/ 
