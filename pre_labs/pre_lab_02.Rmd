...
This code is taking the amount of votes the candidate received on election day and dividing it by the total votes, calculating the percentage of votes that the candidate received on election day. 
...
The arrange(desc) function orders the data in descending order of the percentage of election day votes. Now the candidates with the highest percentage of  votes on election day (2 candidates with 100%) are at the top, and the candidates with the smallest percentage of votes on election day are at the bottom. 
...
This is a pretty interesting answer because it lets us quickly compare who got the highest percentage with who got the lowest percentage. A lot of Republicans got a high percentage of election day votes, while it's mostly Democrats that got the lowest percentage.  
...
On the Tidyverse website, I found str_to_lower, which converts everything to lowercase, and str_to_sentence, which converts it so that only the first letter of a sentence is capitalized. There's also str_to_title, which converts the first letter of every word to uppercase. 
...
This adds a new column to the end that categorizes each donor based on the general amount they contributed (less than $100, more than $2,999, etc)
...
The largest category in terms of dollar amount is "less than $100"! 
...
So that we don't lose it! If we want to come back to this PG County dataset, we need to save it as a new doc(?) and stick it in our environment so we can keep working on it (keep mutating it, making summary statistics, etc). 
...
Now the date format has changed (yyyy-mm-dd instead of m/d/y) and the expenditure_date column is labeled <date>. R knows it's a date! 
...
In the new "month" column, it's just showing the first date. I think this means it's counting everything between June 1 and July 1 as part of that first floor date. 