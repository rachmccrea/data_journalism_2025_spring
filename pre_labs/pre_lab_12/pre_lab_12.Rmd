...
GOT A WEIRD ERROR MESSAGE HERE. R says "Error in open.connection(x, "rb") : cannot open the connection." The rest of the prelab runs fine, though. Not sure what I'm doing wrong here. 
...
#getting an error here: "Error in open.connection(x, "rb") : cannot open the connection." asking terp ai. terp ai is being unhelpful. told me to load rvest but that's not working. 
...
**Task** Run the following code and describe what the `unique` function does to the original list of words. **Answer** It pulls out only the unique words, not repeats. 
...
**Task** Run the following code and describe the output of using the `unnest_tokens` function. **Answer** It creates a dataset where every word has its own row. 
...
**Task** Run the following code and describe what the resulting graphic shows. Is it interesting? **Answer** Shows the top words used in Cardin's press releases as a horizontal bar chart. Uninteresting, it's mostly just "the" and "and" and words like that. Towards the bottom we start getting "project" and "maryland" which is a little more interesting. 
...
Way more interesting! Now the top word is "health," and there are other words like "support, funding, Baltimore, and community." 
...
**Task** Run the following code and describe the results. **Answer** 2022's top 3 are "health, support, and community" while 2024's are "baltimore, support, and funding." Other words like "funding, county, and act" are in both, and so is "health." 
...
**Task** Run the following code and describe the results. **Answer** This is cool! "Amount included" is the top bigram (asking for donations??) but  "health care," "covid 19," and "chesapeake bay" are part of the top 10. So is "mental health," which I think is really interesting. 
...
**Task** Run the following code and describe the results. **Answer** "Health care" is the top bigram, no covid-19, but "chesapeake bay" and "human rights" are still there. "Mental health" isn't in the top 10 -- maybe it was an issue that went along with covid 19 conversations. 
...
**Task** Run the following code and describe the differences between the two results. Which one might be more useful for the press release data? **Answer** The first list is a list of each different word. The second list converts most of the words to "drive", so now we only have two words: driver and drive. This could be useful for organizing words from press release data. Grouping all these words together could let us see how often the press releases talk about driving, instead of counting them separately and losing them in the long list of unique words. 
...
**Task** Run the following code and describe the differences between the two results. Which one is more useful? **Answer** Looks like the first one groups "community" and "communities" which is useful because they're essentially the same. Also groups "fund" and "funding" which is helpful, and shows that he talks about funds more than anything else. I think the lemmatized version is probably more useful because it works kind of like OpenRefine by grouping together similar words. Gives us a clearer picture. 
...
**Task** Run the following code and describe the results. Do any of the sentiments seem incorrect or counter-intuitive? **Answer** "Critical" is coded as negative, but it could be "critical" as in "important" which isn't necessarily negative. Same with "break" -- could mean "vacation" but coded as negative. Looks like this function has trouble with words that have multiple uses.
