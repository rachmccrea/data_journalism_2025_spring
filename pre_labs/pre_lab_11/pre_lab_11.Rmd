...
The dataframe has 7 columns, and each has the same title that they did on the table on the site. The columns have the amount of votes for each candidate by county. They include the different parties and a write-in candidate with 2 total votes. 
We'd definitely want to clean up the column headers, probably with shorter titles and for sure using underscores instead of spaces. 
...
It looks like there are a lot more <li> tags than <ul> tags, I think. 
...
This lists the committee name (I think?) for each <li> tag, which is handy. It looks like some of the <li> tags near the top list some other stuff, too (Representatives, The House Explained, Visitors, etc). I bet those are website headers.
...
Now there's a dataframe with two columns called committees_with_urls, it's got the name of the committee on the left side (example: "Agriculture") and the back half of the url on the right side (example: "https://agriculture.house.gov/"). At the top it's got URLs to those other pages like "Representatives" and "Leadership." 
